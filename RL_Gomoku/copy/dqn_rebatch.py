...
def train_dqn():
    ...
    model = DQN()  # 创建DQN模型
    target_model = DQN()  # 创建目标模型
    target_model.load_state_dict(model.state_dict())  # 将模型的参数复制到目标模型
    target_model.eval()  # 设置目标模型为评估模式
    optimizer = optim.Adam(model.parameters(), lr=0.001)  # 使用Adam优化器，学习率为0.001
    criterion = nn.MSELoss()  # 使用均方误差损失函数
    memory = ExperienceReplay(10000)  # 创建经验回放内存，容量为10000
    ...
    for episode in tqdm(range(algorithm.episodes)):
        ...
        while not done:
            ...
            # AI下棋
                ...
                # 计算目标Q值
                target = reward + 0.99 * torch.max(target_model(next_state_tensor)).item() * (1 - done)
                # 创建一个Q值的副本
                ...
                # 将经验添加到经验回放内存中
                memory.push((state_tensor, action, target_f, next_state_tensor, done))

                # 从经验回放内存中随机采样一批经验
                batch = memory.sample(32)
                state_batch, action_batch, target_batch, next_state_batch, done_batch = zip(*batch)
                ...
                # 计算损失
                loss = criterion(q_values.gather(1, action_batch.unsqueeze(1)), target_batch)
